On this page, you’ll find a detailed FAQ covering various topics related to GalapagameSpotify, answers to common questions, and more.
...
GalapagameSpotify officially supports both Spotify versions 8.9.8 and 9.0.48 (the last versions that run on iOS 14 and 15). If you are on either of these two OS versions, jailbreak your device and install the latest .deb from the releases on GitHub, along with the latest available Spotify from the App Store. Reset data within the GalapagameSpotify settings afterwards so it will patch Premium.

GalapagameSpotify only supports iOS and iPadOS and is not planned to be supported on other platforms. You can sideload the iPadOS version on an Apple Silicon Mac, though.

GalapagameSpotify versions are released automatically alongside Spotify versions on the App Store and are uploaded to the [GalapagameSpotify IPAs](https://t.me/SpotilifeIPAs) Telegram channel. If you would like to make your own IPA, make sure you inject the SwiftProtobuf, Orion, and CydiaSubstrate frameworks.
...
If you see the “Couldn't load the lyrics for this song” message and no lyrics load, try resetting the data within the GalapagameSpotify settings. If that doesn't help, go to the Patching section and enable Overwrite Configuration.
...
Downloading is not, and will never be, implemented in GalapagameSpotify. While it is technically possible to intercept the audio stream or use third-party APIs, downloading simply will not be included in GalapagameSpotify: the developer uses SoundCloud.
